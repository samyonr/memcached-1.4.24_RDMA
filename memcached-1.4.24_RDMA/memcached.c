...
static void settings_init(void) {
    ...
    settings.shared_malloc_slabs = false;
    settings.shared_malloc_slabs_key = NULL;
    settings.shared_malloc_slabs_lists = false;
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("shared_malloc_slabs", "%s", settings.shared_malloc_slabs ? "yes" : "no");
    APPEND_STAT("shared_malloc_slabs_key", "%s", settings.shared_malloc_slabs_key ? settings.shared_malloc_slabs_key : "NULL");
    APPEND_STAT("shared_malloc_slabs_lists", "%s", settings.shared_malloc_slabs_lists ? "yes" : "no");
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    fprintf(stderr, "%d: process_get_command 10\n", c->sfd);
    c->icurr = c->ilist;
    c->ileft = i;
    if (return_cas) {
        c->suffixcurr = c->suffixlist;
       ...
       case 'F' :
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case SHARED_MALLOC_SLABS:
                ...
                break;
            case SHARED_MALLOC_SLABS_LISTS:
                if (subopts_value == NULL) {
